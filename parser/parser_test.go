...
func TestEncode_DictContainingList(t *testing.T) {
	// Encode a dict whose value is a list of strings
	// Expected: d + 5:files + l5:a.txt5:b.txte + e
	input := map[string]any{
		"files": []any{"a.txt", "b.txt"},
	}
	got := Encode(input)
	want := "d5:filesl5:a.txt5:b.txtee"
	if got != want {
		t.Errorf("Encode(dict with list value) = %q, want %q", got, want)
	}
}

func TestEncode_ListContainingDict(t *testing.T) {
	// Encode a list that contains a dict
	input := []any{map[string]any{"key": "val"}}
	got := Encode(input)
	want := "ld3:key3:valee"
	if got != want {
		t.Errorf("Encode(list with dict) = %q, want %q", got, want)
	}
}

func TestEncode_NilValue(t *testing.T) {
	// Encoding nil (unsupported type) should return empty string
	got := Encode(nil)
	if got != "" {
...
func TestRoundTrip_TorrentLikeFullDict(t *testing.T) {
	// Full torrent-like bencoded string with announce + info sub-dict; keys sorted
	original := "d8:announce27:http://example.com/announce4:infod6:lengthi512e4:name7:foo.iso12:piece lengthi128e6:pieces20:BBBBBBBBBBBBBBBBBBBBee"
	decoded := Decode(original)
	reencoded := Encode(decoded)
	if reencoded != original {
		t.Errorf("round-trip full torrent dict: got %q, want %q", reencoded, original)
	}
}

func TestDecode_IntegerOne(t *testing.T) {
	// Regression: single-digit integers (including 1) decoded correctly
	got := Decode("i1e")
	v, ok := got.(int)
	if !ok {
		t.Fatalf("expected int, got %T", got)
	...
	if v != 1 {
		t.Errorf("Decode(\"i1e\") = %d, want 1", v)
	}
}