...
func TestNewTorrentFile_ThreePieces(t *testing.T) {
	// 60 bytes = exactly 3 pieces, boundary check
	pieces := strings.Repeat("A", 20) + strings.Repeat("B", 20) + strings.Repeat("C", 20)
	input := "d8:announce35:http://tracker.example.com/announce4:infod6:lengthi3000e4:name8:test.iso12:piece lengthi256e6:pieces60:" + pieces + "ee"
	tf, err := NewTorrentFile(input)
	if err != nil {
		t.Fatalf("expected no error for three pieces, got: %v", err)
	}
	if len(tf.PieceHashes) != 3 {
		t.Fatalf("expected 3 piece hashes, got %d", len(tf.PieceHashes))
	}
	for i := 0; i < 20; i++ {
		if tf.PieceHashes[2][i] != 'C' {
			t.Errorf("PieceHashes[2][%d] = %d, want 'C'", i, tf.PieceHashes[2][i])
		}
	}
}

func TestNewTorrentFile_InfoHash_Deterministic(t *testing.T) {
	// Calling NewTorrentFile twice with the same input must yield the same InfoHash
	input := validBencode()
	tf1, err := NewTorrentFile(input)
	if err != nil {
		t.Fatalf("first call failed: %v", err)
	}
	tf2, err := NewTorrentFile(input)
	if err != nil {
		t.Fatalf("second call failed: %v", err)
	}
	if tf1.InfoHash != tf2.InfoHash {
		t.Errorf("InfoHash is not deterministic: %x != %x", tf1.InfoHash, tf2.InfoHash)
	}
}

func TestNewTorrentFile_DifferentInputsDifferentInfoHash(t *testing.T) {
	// Two torrent files with different names must have different InfoHashes
	input1 := "d8:announce35:http://tracker.example.com/announce4:infod6:lengthi1000e4:name8:test.iso12:piece lengthi256e6:pieces20:AAAAAAAAAAAAAAAAAAAAee"
	input2 := "d8:announce35:http://tracker.example.com/announce4:infod6:lengthi1000e4:name8:test.mp412:piece lengthi256e6:pieces20:AAAAAAAAAAAAAAAAAAAAee"
	tf1, err := NewTorrentFile(input1)
	if err != nil {
		t.Fatalf("first NewTorrentFile failed: %v", err)
	}
	tf2, err := NewTorrentFile(input2)
	if err != nil {
		t.Fatalf("second NewTorrentFile failed: %v", err)
	}
	if tf1.InfoHash == tf2.InfoHash {
		t.Errorf("expected different InfoHashes for different torrents, got same: %x", tf1.InfoHash)
	}
}